# Anna services booking mapping

The website has no verified per-service MaSe URLs. To avoid fake or placeholder
links, every new Anna service uses the existing server-side `/book` handoff to
the general MaSe booking destination. The following services therefore remain
pending exact MaSe item mapping:

## Hair Atelier

- MEND AI Scalp Analysis
- Hair & Scalp Wellness Assessment
- Ladies Signature Cut
- Gentleman’s Precision Cut
- Fringe Refresh
- Kids’ Signature Cut — Under 12
- Signature Colour
- Root Refresh
- Dimension Highlights
- Signature Balayage
- Korean Soft Wave
- Classic Texture Perm
- Ladies Texture Wave
- Men’s Texture Perm

## Hair & Scalp Recovery

- Scalp Detox Ritual
- Hydra Scalp Ritual
- Hair Growth Ritual
- Scalp Recovery Ritual
- Collagen Hair Ritual
- Keratin Smooth Ritual
- Glass Hair Premium Signature

## Scalp & Mind Wellness

- Express Head Spa
- Signature Head Spa
- Aroma Healing Head Spa
- MEND Signature Head Ritual

## Skin Aesthetics

- Skin Barrier Repair
- Hydra Restore
- Glass Glow Signature
- Lift & Firm Recovery
- Acne Recovery
- Pigment Reset
- Korean Glass Booster
- Collagen Restore
- Advanced Aesthetics
- MEND Bridal Journey
- MEND Bio-Recovery Signature — Face + Scalp

## Body Wellness

- Express Neck & Shoulder Therapy
- Meridian Therapy
- Qi & Circulation Therapy
- Lymphatic Wellness Therapy
- Body Reset Therapy
- Express Relaxation Massage
- Aroma Healing Massage
- Deep Sleep Massage
- MEND Signature Body Ritual

The protected Men’s Grooming and Nails & Semi-Permanent Beauty entries retain
their existing central booking behavior.
